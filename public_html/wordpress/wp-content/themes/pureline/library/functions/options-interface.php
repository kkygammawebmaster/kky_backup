...
function pureline_fields() {
	...
	foreach ($options as $value) {
      ...
      if ( $value['type'] == "open-tab" || $value['type'] == "close-tab") { } else {
		...
		// Uploader
		case "upload":
			$output .= pureline_medialibrary_uploader( $value['id'], $val, null ); // New AJAX Uploader using Media Library	
		break;
		...
		case 'typography':	
			...
			$faces = pureline_recognized_font_faces();
			...
			/* Font Style */
			$styles = pureline_recognized_font_styles();
		...
		case 'background':
			...
			$output .= pureline_medialibrary_uploader( $value['id'], $background['image'], null, '',0,'image');
			...
			$repeats = pureline_recognized_background_repeat();
			...
			$positions = pureline_recognized_background_position();
			...
			$attachments = pureline_recognized_background_attachment();
	...
	// Info
		case "info":
			...
			if ( $value['desc'] ) {
				$output .= apply_filters('pureline_sanitize_info', $value['desc'] ) . "\n";
			}
